...
dnn_wcw_model.compile(optimizer=opt, loss='binary_crossentropy', metrics=['accuracy'])

wcw_model_checkpoint = keras.callbacks.ModelCheckpoint(filepath='/Code/Models/DNN_AbsDiff_wCW.keras', save_best_only=True, monitor='val_accuracy', mode='max', verbose=1)
...
wcw_history = dnn_wcw_model.fit(X_train, y_train, epochs=50, validation_data=(X_val, y_val), callbacks=[wcw_model_checkpoint, wcw_model_early_stopping])
...
dnn_cw_model.compile(optimizer=opt, loss='binary_crossentropy', metrics=['accuracy'])

cw_model_checkpoint = ModelCheckpoint(filepath='/Code/Models/DNN_AbsDiff_CW.keras', save_best_only=True, monitor='val_accuracy', mode='max', verbose=1 )
...
cw_history = dnn_cw_model.fit(X_train, y_train, epochs=50, class_weight=class_weight, validation_data=(X_val, y_val), callbacks=[cw_model_checkpoint, cw_model_early_stopping])
...
dnn_cb_model.compile(optimizer=opt, loss='binary_crossentropy', metrics=['accuracy'])


cb_model_checkpoint = ModelCheckpoint(filepath='/Code/Models/DNN_AbsDiff_CB.keras', save_best_only=True, monitor='val_accuracy', mode='max', verbose=1 )
...
cb_history = dnn_cb_model.fit(cb_train_patches, cb_train_labels, epochs=50, class_weight=class_weight, validation_data=(X_val, y_val), callbacks=[cb_model_checkpoint, cb_model_early_stopping])
...
test_loss, test_acc = dnn_wcw_model.evaluate(X_test, y_test)
test_acc  = test_acc * 100

predictions = dnn_wcw_model.predict(X_test)
...
plt.grid(True)
precision_recall_curve_path = '/Code/Plots/DNN_AbsDiff_wCW_precision_recall_curve.png'
...
test_loss, test_acc = dnn_cw_model.evaluate(X_test, y_test)
test_acc  = test_acc *100

predictions = dnn_cw_model.predict(X_test)
...
plt.legend()
plt.grid(True)
precision_recall_curve_path = '/Code/Plots/DNN_AbsDiff_CW_precision_recall_curve.png'
...
test_loss, test_acc = dnn_cb_model.evaluate(X_test, y_test)
test_acc  = test_acc *100

predictions = dnn_cb_model.predict(X_test)
predicted_labels = (predictions > 0.5).astype(int).ravel()
true_labels = y_test.ravel()  
...
plt.legend()
plt.grid(True)
precision_recall_curve_path = '/Code/Plots/DNN_AbsDiff_CB_precision_recall_curve.png'
...
misclass_En_csv_path = '/Code/Results/Precision_Ensemble_DNN_AbsDiff_misclassified_patches.csv'
...
misclass_En_csv_path = '/Code/Results/Average_Ensemble_DNN_AbsDiff_misclassified_patches.csv'
misclassified_indexes = np.where(predicted_classes != true_labels)[0]
