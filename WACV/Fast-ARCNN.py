...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), os.path.join(RESULTS_DIR, 'Best_FastARCNN_Model2.pth'))
        print(f"New best model saved with validation loss: {val_loss:.4f}")


model.load_state_dict(torch.load(os.path.join(RESULTS_DIR, 'Best_FastARCNN_Model2.pth')))
model.eval()
