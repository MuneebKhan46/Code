...
with strategy.scope():
    ...
    model_checkpoint = ModelCheckpoint(filepath='/ghosting-artifact-metric/WACV/Model/lowFreq_DnCNN.keras', save_best_only=True, monitor='val_loss', mode='min', verbose=1)
...
with tf.keras.utils.custom_object_scope({'sum_squared_error': sum_squared_error}):
    model = tf.keras.models.load_model("/ghosting-artifact-metric/WACV/Model/lowFreq_DnCNN.keras")
...
results.append({'Model': 'DnCNN', 'Artifact Type': 'Low Freq', 'PSNR': avg_psnr, 'SSIM': avg_ssim})
