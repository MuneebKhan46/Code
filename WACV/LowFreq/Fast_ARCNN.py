...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), os.path.join(model_dir, 'LowFreq_FastARCNN_Model.pth'))
        print(f"New best model saved with validation loss: {val_loss:.4f}")




model.load_state_dict(torch.load(os.path.join(model_dir, 'LowFreq_FastARCNN_Model.pth')))
model.eval()
...
image_save_dir = os.path.join(Results_dir, 'images/FastAR-CNN/')
