...
class FastARCNN(nn.Module):
    def __init__(self):
        ...
        self.base = nn.Sequential(
            ...
            nn.PReLU(),
            nn.Conv2d(64, 32, kernel_size=7, stride=2, padding=3),  
            nn.PReLU(),
            nn.Conv2d(32, 16, kernel_size=1),
            nn.PReLU()
        )
        self.last = nn.ConvTranspose2d(16, 3, kernel_size=5, stride=2, padding=2, output_padding=1)  
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), os.path.join(model_dir, 'HighFreq_FastARCNN_Model.pth'))
        print(f"New best model saved with validation loss: {val_loss:.4f}")




model.load_state_dict(torch.load(os.path.join(model_dir, 'HighFreq_FastARCNN_Model.pth')))
model.eval()
...
def visualize_and_save_patches(original, denoised, restored, idx):
    ...
    original_file = os.path.join(image_save_dir, f"FastARCNN_original_patch_{idx}.png")
    denoised_file = os.path.join(image_save_dir, f"FastARCNN_denoised_patch_{idx}.png")
    restored_file = os.path.join(image_save_dir, f"FastARCNN_restored_patch_{idx}.png")
...
image_save_dir = os.path.join(Results_dir, 'images/FastAR-CNN_High/')
...
results.append({'Model': 'Fast AR-CNN', 'Artifact Type': 'High Freq', 'PSNR': avg_psnr, 'SSIM': avg_ssim})
